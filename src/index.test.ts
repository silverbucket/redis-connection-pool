...
class TestRedisConnectionPool extends RedisConnectionPool {
  createGenericPoolConstructor() {
    this.genericPoolConstructor = createPoolFake;
  }
}
...
describe("redisConnectionPoolFactory", () => {
  ...
  beforeEach(async () => {
    pool = await redisConnectionPoolFactory(
      "foo",
      {
        max_clients: 99,
        redis: {
